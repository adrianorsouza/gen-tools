// Source: https://github.com/jherr/njs13-n-plus-one/blob/main/tester/index.js
...
(async () => {
  ...
  $('script').each((i, el) => {
    ...
    if (scriptText.startsWith('self.__next_f.push([1,"')) {
      try {
        const mainScriptBlock = scriptText.replace('self.__next_f.push(', '').replace(/\)$/, '');
  ...
  console.log(
    `            HTML: ${convertToKb(totalSize - scriptSize)} - ${percent((totalSize - scriptSize) / totalSize)}`
  );
  console.log(`            JSON: ${convertToKb(scriptSize)} - ${percent(scriptSize / totalSize)}`);
  console.log(`JSON Client Data: ${convertToKb(clientData)} - ${percent(clientData / scriptSize)}`);
})();
